...
use crate::types::state::ColumnState;
...
pub fn TaskColumn(column: ColumnState) -> impl IntoView {
    ...
    let column_name = Signal::derive(move || column.column_type.name());
    let wip_limit = Signal::derive(move || column.wip_limit.get());
    let tasks = Signal::derive(move || column.tasks.get());
    let task_count = Signal::derive(move || tasks.get().len());

    let is_wip_limit_reached = Signal::derive(move || {
        wip_limit
            .get()
            .is_some_and(|limit| task_count.get() >= limit as usize)
    });
    ...
    let move_task_to_column_end = Callback::new(move |dragged_item: DraggableItemDto| {
        let action = if dragged_item.source_column_type == column.column_type {
        ...
        if app
            .boards
            .move_task_in_current_board(&dragged_item.task_id, &column.column_type, None)
        {
            (measurement)();
        }
    });
